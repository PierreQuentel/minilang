...
var DefCtx = function(context){
        ...
        switch(token){
            ...
            case 'eol':
                //if(context.has_args){
                    return context.parent
                //}else{
                //    $_SyntaxError(context, "no () after def")
                //}
        }
        $_SyntaxError(context, 'unexpected token ' + token)
    }
    ...
    this.to_js = function(func_name){
        if(this.has_args){
            return `locals.${this.name} = function(${this.args.join(', ')})`
        }else{
            return `locals.${this.name} = (function(${this.args.join(', ')})`
        }
    }
...
function ImportCtx(context){
    ...
    this.to_js = function(){
        var js = 'var module = $M.imported["' + this.modname +
            ...
            this.modname +'")};locals["' + this.modname + '"] = module'
        return js
    }
}
...
var LoopCtx = function(context){
    ...
    this.to_js = function(){
        if(this.tree.length == 0){
            ...
            var js = "var _ = " + this.tree[0].to_js() + ';' +
                     'if(typeof _ == "number"){_ = $M.range(0, _)};' +
                     'if(typeof _[Symbol.iterator] !== "function")' +
                     '{throw $M.Error("loop argument is not iterable")};'
            return js + "for(const " + this.varname + " of _)"
        }
    }
}
            ...
            if(indent > current.indent){
                if(context !== null){
                    if($indented.indexOf(context.tree[0].type) == -1){
                        $pos = pos
                        console.log("indentation", current)
